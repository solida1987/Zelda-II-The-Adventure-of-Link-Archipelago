...
class Zelda2World(World):
    ...
    def create_regions(self) -> None:
        ...
        for loc in self._included():
            ...
            spot = Zelda2Location(self.player, loc.name, loc.id, region)
            spot.item_rule = self._placement_rule(loc.region_key)
            region.locations.append(spot)
    ...
    def _placement_rule(self, region_key: str):
        def rule(item, key=region_key):
            # Another world's item is a Blue Jar here, and jars go anywhere.
            if item.player != self.player:
                return True
            return kind_allowed_at(ITEM_BY_NAME[item.name].key, key)
        return rule
    ...
    def generate_output(self, output_directory: str) -> None:
        # Ownership decides the byte, not the name: another zelda2 player's
        # Palace Key must become a Blue Jar here, not a real key.
        placements = {
            loc.name: (loc.item.name if loc.item.player == self.player
                       else "__other_world__")
