...
from .Regions import kind_allowed_at
...
# The same cartridge with its header, which is what the patch manifest records.
# Two forms circulate -- iNES v1 and NES 2.0 -- and they differ only in header
# bytes 7, 10 and 15. Both are the same game, so both are accepted; verify()
# is the real gate and ignores the header entirely.
HEADERED_MD5 = [
    "764d36fa8a2450834da5e8194281035a",  # iNES v1
    "e6dd4104fa46eb4bef2cb52dc341dd38",  # NES 2.0
]
HEADER_LEN = 16
...
def build_tokens(placements: dict[str, str], options) -> list[tuple[int, bytes]]:
    ...
    for loc_name, item_name in placements.items():
        ...
        if item is not None and not kind_allowed_at(item.key, loc.region_key):
            raise PatchError(
                f"{item_name} cannot sit at {loc_name}: the game has no "
                "graphics or pickup for it in that scene")
